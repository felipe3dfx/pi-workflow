...
} from "../extensions/approved-revision-publication.ts";
...
function approvedRevision(overrides = {}) {
	const payload = {
		...
		sourceDraftDigest: "f".repeat(64),
		authority: actor,
		affectedIssues: [
			{
				id: "ILA-2296",
				previousDescription: "Spec anterior",
				previousRevision: "rev-parent-1",
				nextDescription: "Spec vigente revisado",
			},
			{
				id: "ILA-2317",
				previousDescription: "Ticket anterior",
				previousRevision: "rev-ticket-1",
				nextDescription: "Ticket vigente revisado",
			},
		],
		sourceComment: {
			kind: "product-revision",
	...
	const digest = digestCanonicalValue({
		...
		payload: {
			...payload,
			sourceComment: {
				...payload.sourceComment,
				body: payload.sourceComment.body.replaceAll("DIGEST", "placeholder"),
			},
			decisionGap: payload.decisionGap
				? {
						...payload.decisionGap,
						body: payload.decisionGap.body.replaceAll("DIGEST", "placeholder"),
					}
				: undefined,
		},
	});
	return {
		...
		sourceComment: {
			...payload.sourceComment,
			body: payload.sourceComment.body.replaceAll("DIGEST", digest),
		},
		decisionGap: payload.decisionGap
			? {
					...payload.decisionGap,
					body: payload.decisionGap.body.replaceAll("DIGEST", digest),
				}
			: undefined,
	};
}
...
function memory() {
	...
	return {
		...
		read: async () => current,
		create: async (value) =>
			(current = { revision: "r1", value: structuredClone(value) }),
		compareAndSwap: async (revision, value) => {
			assert.equal(revision, current.revision);
			current = {
				revision: `r${Number(current.revision.slice(1)) + 1}`,
...
function linear(overrides = {}) {
	const issues = new Map([
		[
			...
			{
				id: "ILA-2296",
				description: "Spec anterior",
				updatedAt: "rev-parent-1",
				workflow: {
					state: "In Progress",
					assignee: "owner-1",
					cycle: "cycle-1",
					labels: ["Felipe Gonzalez"],
					project: "Pi Workflow harness",
				},
			},
		],
		[
		...
		gateway: {
			...
			saveComment: async (input) => {
				assert.deepEqual(
					...
					["body", "issueId"],
					"comments must not carry workflow mutation fields",
				);
				calls.push({
					op: "saveComment",
				...
				const entry = {
					id: `comment-${(comments.get(input.issueId) ?? []).length + 1}`,
					body: input.body,
				};
				comments.set(input.issueId, [
					...(comments.get(input.issueId) ?? []),
					entry,
				]);
				if (overrides.mutateWorkflowOn === "comment")
					issues.get(input.issueId).workflow.state = "Done";
			...
			saveIssue: async (input) => {
				assert.deepEqual(
					Object.keys(input).sort(),
					["description", "id"],
					"issue updates must only send explicit description fields",
				);
				calls.push({
					op: "saveIssue",
					id: input.id,
				...
				if (
					overrides.mutateWorkflowOn === "issue" ||
					(overrides.mutateAndInterruptIssueOnce && !overrides.interruptedIssue)
				)
					updated.workflow.cycle = "cycle-2";
				issues.set(input.id, updated);
				if (
					overrides.mutateAndInterruptIssueOnce &&
					!overrides.interruptedIssue
				) {
...
function deps({
	...
	return {
		...
		readApprovedRevision: async (definitionId, digest) =>
			definitionId === revision.definitionId && digest === revision.digest
...
function artifactMemory() {
	...
	return {
		...
		write: async (_project, topic, content, expectedRevision) => {
			...
			if (versions.at(-1)?.revision !== expectedRevision)
		...
		readRevision: async (_project, topic, target) =>
			bytes.get(topic)?.find((entry) => entry.revision === target)?.content,
	};
}
...
test("normalizes legacy schema-v1 manifests that predate description claims", async () => {
	...
	const identity = {
		definitionId: revision.definitionId,
		digest: revision.digest,
		affectedIssueIds: revision.affectedIssues.map(({ id }) => id).sort(),
	};
	...
	const store = createApprovedRevisionPublicationManifestStore({
		persistence: {
			...
			compareAndSwap: async (_revision, value) =>
				(current = { revision: "r2", value: structuredClone(value) }),
		},
	});
...
test("publishes an approved Spanish multi-issue revision after validating every previous description", async () => {
	...
	assert.deepEqual(
		subject.linearFake.calls.map((call) => call.op),
		["saveComment", "saveComment", "saveComment", "saveIssue", "saveIssue"],
	);
	assert.equal(
		subject.linearFake.issues.get("ILA-2296").description,
		"Spec vigente revisado",
	);
	assert.equal(
		subject.linearFake.issues.get("ILA-2317").description,
		"Ticket vigente revisado",
	);
...
test("blocks when comment or description writes mutate workflow fields", async () => {
	for (const mutation of ["comment", "issue"]) {
		const subject = deps({
			linearFake: linear({ mutateWorkflowOn: mutation }),
		});
		const result = await publishApprovedRevision(subject);
...
test("recovery compares workflow to the durable pre-write claim after mutation and interruption", async () => {
	const subject = deps({
		linearFake: linear({ mutateAndInterruptIssueOnce: true }),
	});
...
test("upgrades a legacy description claim with a workflow digest before retrying its write", async () => {
	...
	assert.match(
		subject.persistence.value().descriptionClaims[0].workflowDigest,
		/^[a-f0-9]{64}$/,
	);
	assert.equal(
		subject.linearFake.issues.get("ILA-2296").description,
...
test("fails closed for a legacy description claim when the description already changed", async () => {
	...
	subject.linearFake.issues.set("ILA-2296", {
		...current,
		description: "Spec vigente revisado",
		updatedAt: `${current.updatedAt}:external`,
	});
...
test("is idempotent and treats same reference with different body as conflict", async () => {
	const subject = deps();
	assert.equal(
		(await publishApprovedRevision(subject)).status,
		"revision-published",
	);
	const retry = await publishApprovedRevision(subject);
	assert.equal(retry.status, "revision-published");
	assert.equal(
		subject.linearFake.calls.filter((call) => call.op === "saveComment").length,
		3,
	);
	assert.equal(
		subject.linearFake.calls.filter((call) => call.op === "saveIssue").length,
		2,
	);

	const conflicting = deps();
	conflicting.linearFake.comments.set("ILA-2296", [
		{
			id: "comment-1",
...
test("recovers staged publication without duplicating comments or issue updates", async () => {
	...
	assert.equal(
		linearFake.calls.filter((call) => call.op === "saveComment").length,
		3,
	);
	assert.equal(
		linearFake.calls.filter((call) => call.op === "saveIssue").length,
		1,
	);
	...
	const retry = deps({
		persistence,
		linearFake,
		revision: await first.readApprovedRevision(
			first.definitionId,
			first.digest,
		),
	});
	assert.equal(
		(await publishApprovedRevision(retry)).status,
		"revision-published",
	);
	assert.equal(
		linearFake.calls.filter((call) => call.op === "saveComment").length,
...
test("blocks approval drift because current Owner actor is bound to the digest", async () => {
	const subject = deps({
		currentActor: { ...actor, authorityRevision: "owner-r2" },
	});
	const result = await publishApprovedRevision(subject);
...
test("rejects content forged under a caller-chosen approved digest", async () => {
	...
	const forged = {
		...revision,
		affectedIssues: revision.affectedIssues.map((issue, index) =>
			index === 0
				? { ...issue, nextDescription: "Descripción no aprobada" }
				: issue,
		),
	};
...
test("does not treat an externally matching next description as recorded recovery", async () => {
	...
	linearFake.issues.set("ILA-2296", {
		...linearFake.issues.get("ILA-2296"),
		description: "Spec vigente revisado",
		updatedAt: "unapproved-revision",
	});
...
test("drafts an approved revision by reading current Linear descriptions before persisting Engram draft", async () => {
	const linearFake = linear();
	const store = createApprovedRevisionStore({
	...
	const result = await draftApprovedRevision({
		...
		input: {
			...
			sourceCommentBody:
				"Revisión aprobada para el flujo.\n\nReferencia de flujo: product-revision:{{digest}}",
			decisionGap: {
				issueId: "ILA-2317",
				body: "Brecha de decisión documentada.\n\nReferencia de flujo: decision-gap:{{digest}}",
			},
	...
	assert.deepEqual(
		result.revision.affectedIssues.map(
			({ id, previousDescription, previousRevision, nextDescription }) => ({
				id,
				previousDescription,
				previousRevision,
				nextDescription,
	...
	assert.match(
		result.revision.sourceComment.body,
		new RegExp(
			`Referencia de flujo: product-revision:${result.revision.digest}`,
		),
	);
...
test("approves a drafted revision with the current Owner and rejects stale draft snapshots", async () => {
	...
	const approved = await approveDraftedRevision({
		definitionId: "definition-1",
		digest: ready.revision.digest,
	...
	assert.equal(
		(await store.readApproved("definition-1", approved.revision.digest))
			.authority.actorId,
		"owner-1",
	);

	const staleStore = createApprovedRevisionStore({
		store: artifactMemory(),
		project: "pi-workflow",
		topic: "workflow/define-product",
	});
	const staleReady = await draftApprovedRevision({
		gateway: linearFake.gateway,
		store: staleStore,
		input: {
			definitionId: "definition-1",
	...
	const stale = await approveDraftedRevision({
		definitionId: "definition-1",
		digest: staleReady.revision.digest,
		currentActor: async () => actor,
		gateway: linearFake.gateway,
		store: staleStore,
	});
...
test("recovers an issue update only after durable per-issue intent is recorded", async () => {
	...
	const guardedPersistence = {
		...
		compareAndSwap: async (revision, value) => {
			if (
				value.stage === "describing" &&
				value.descriptions.includes("ILA-2296") &&
				!failedCompletionRecord
			) {
	...
	assert.equal((await publishApprovedRevision(subject)).status, "blocked");
	assert.equal(
		linearFake.issues.get("ILA-2296").description,
		"Spec vigente revisado",
	);
	assert.equal(
		persistence
			.value()
			.descriptionClaims.some((claim) => claim.issueId === "ILA-2296"),
		true,
	);
	assert.equal(persistence.value().descriptions.includes("ILA-2296"), false);

	const retry = deps({
		persistence: guardedPersistence,
		linearFake,
		revision: await subject.readApprovedRevision(
	...
	assert.equal(
		linearFake.calls.filter(
			(call) => call.op === "saveIssue" && call.id === "ILA-2296",
		).length,
		1,
	);
});
...
test("blocks an external identical edit for an unclaimed issue during staged recovery", async () => {
	...
	let manifest = await subject.manifest.prepare({
		definitionId: subject.definitionId,
		digest: subject.digest,
		affectedIssueIds: ["ILA-2296", "ILA-2317"],
	});
	manifest = await subject.manifest.advance(
	...
	await subject.manifest.record(manifest.operationId, "describing", {
		descriptionClaims: [
			{ issueId: "ILA-2296", previousRevision: "rev-parent-1" },
		],
	});
	linearFake.issues.set("ILA-2317", {
		...linearFake.issues.get("ILA-2317"),
		description: "Ticket vigente revisado",
		updatedAt: "external-edit",
	});
...
test("carries one shared revision lease through every comment and description mutation", async () => {
	const revision = approvedRevision();
	const lease = {
		...
		interactiveDecisions: {
			async authorizeEffect(candidate) {
				...
				return {
					...
					lease: candidate,
					manifest: {
						ref: `workflow://define-product/${revision.definitionId}/approved-revision-publication/${revision.sourceDraftDigest}`,
					},
				};
			},
		},
	...
	assert.equal(
		(await publishApprovedRevision(dependencies)).status,
		"revision-published",
	);
	assert.equal(effects.length, 6);
	assert.ok(
		effects.every((candidate) => candidate.executionId === lease.executionId),
	);
	assert.deepEqual(dependencies.persistence.value().executionBinding, {
