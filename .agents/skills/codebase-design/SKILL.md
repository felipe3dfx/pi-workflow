...
The vocabulary never yields. The terms below are the shared language, and a project that redefines them loses the only thing this skill provides.
...
- **Prefer the highest useful existing seam.** **Highest** is nearest the public surface. **Useful** means the behaviour under test is fully exercised through it and the test fails at its starting commit. Always prefer a seam that already exists; introduce a new one only when two adapters justify it.
...
- **Exploring alternative interfaces**, see [DESIGN-IT-TWICE.md](references/DESIGN-IT-TWICE.md): design the interface several radically different ways, then compare on depth, locality, and seam placement.
