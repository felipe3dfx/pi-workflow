...
const PATTERNS: Array<{
	...
	},
	{
		name: "async-no-await",
		pattern:
			/async\s+(?:function\s+\w+|\w+\s*=\s*async)\s*\([^)]*\)\s*(?::\s*\w+)?\s*\{[^}]*\}/g,
		severity: "medium",
		category: "Async",
	...
	},
	{
		...
		category: "Async",
		message:
			"Unnecessary Promise constructor (async executor or can be simplified)",
	...
	},
	{
		...
		category: "Iteration",
		message:
			"for...in iterates over all enumerable properties, not just indices",
	...
	for (let lineNum = 0; lineNum < lines.length; lineNum++) {
		...
		for (const pattern of PATTERNS) {
			...
			for (
				...
				if (
					isCommentLine &&
					!["todo-comment", "ts-ignore", "ts-nocheck"].includes(pattern.name)
				) {
					continue;
				}
