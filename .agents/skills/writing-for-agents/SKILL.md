...
This skill is entirely reference, consulted when writing a document rather than followed step by step, so it claims the style guide's exemption from the body word maximum.
...
Repository policy does take precedence over any concrete recommendation here. [The skill style guide](../../../docs/skill-style-guide.md) governs location, frontmatter shape, licensing, provenance, and the word budget; where it and this document disagree on a specific rule, the style guide decides and this skill informs the reasoning behind it.
